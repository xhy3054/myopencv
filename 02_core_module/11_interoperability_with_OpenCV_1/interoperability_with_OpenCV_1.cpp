...
//! [start]如果定义了混合使用就包含c版本的头文件，并进行一些ｃ接口的功能展示
...
{
    help(argv[0]);
    const char* imagename = argc > 1 ? argv[1] : "../../data/wechat.jpg";

#ifdef DEMO_MIXED_API_USE
    Ptr<IplImage> IplI(cvLoadImage(imagename));      // Ptr<T> is a safe ref-counting pointer class智能指针管理c接口的数据结构的内存，无需再自己释放
    if(!IplI)
    {
...
//! [start]
    ...
    //! [new]
    // convert image to YUV color space. The output image will be created automatically.变换颜色空间
    Mat I_YUV;
    cvtColor(I, I_YUV, COLOR_BGR2YCrCb);

    vector<Mat> planes;    // Use the STL's vector structure to store multiple Mat objects
	//将YUV颜色空间的图像分离成３副图像并存储到一个ｖｅｃｔｏｒ里
