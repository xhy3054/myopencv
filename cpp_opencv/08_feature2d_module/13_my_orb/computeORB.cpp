...
int main(int argc, char **argv)
{
    ...
    std::cout << "used time: " << time_used.count() << std::endl;
...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints)
{
    ...
    for (auto &kp : keypoints)
    {
        ...
        kp.angle = atan2(m01, m10) * 180 / M_PI; // compute kp.angle
        // END YOUR CODE HERE
    }
    return;
}
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc)
{
    for (auto &kp : keypoints)
    {
        ...
        {
            // START YOUR CODE HERE (~7 lines)
            float sin_theta = sin(kp.angle * M_PI / 180);
            float cos_theta = cos(kp.angle * M_PI / 180);

            cv::Point2f pp = cv::Point2f(cos_theta * ORB_pattern[i * 4] - sin_theta * ORB_pattern[i * 4 + 1], sin_theta * ORB_pattern[i * 4] + cos_theta * ORB_pattern[i * 4 + 1]) + kp.pt;
            cv::Point2f qq = cv::Point2f(cos_theta * ORB_pattern[i * 4 + 2] - sin_theta * ORB_pattern[i * 4 + 3], sin_theta * ORB_pattern[i * 4 + 2] + cos_theta * ORB_pattern[i * 4 + 3]) + kp.pt;
